...
@bags = {}
get_input.each do |line|
  outer = line.slice! /\S+ \S+/
  @bags[outer] = line.scan /(\d+) (\S+ \S+) bag/
end

def part1
  contains_sg = -> c {
    return true if c == 'shiny gold'
    @bags[c].any? { |_, c| contains_sg[c] }
  }

  @bags.count { |c, _| contains_sg[c] } - 1
end

def part2
  tally = -> c {
    @bags[c].sum { |n, c| tally[c] * n.to_i } + 1
  }
