...
def part1
  ...
  passports.count { |p| fields.all? { |f| p[f] } }
end

def part2
  passports = get_input "\n\n"
  passports.count { |p|
    data = p.split.map { |d| d.split ?: }.to_h
    hgt = data['hgt'] || ''
    (1920..2002) === data['byr'].to_i &&
    (2010..2020) === data['iyr'].to_i &&
    (2020..2030) === data['eyr'].to_i &&
    (hgt['cm'] ? 150..193 : 59..76) === hgt.to_i &&
    (data['hcl'] || '')[/^#\h{6}$/] &&
